// lib/motor.js — Puente al motor de análisis Pickazo MX (Python + RAG)
// Camino B: pickazoapp le pide TODO al motor (partidos, tabla, jugadores, análisis).

const MOTOR_URL = process.env.MOTOR_URL;
const MOTOR_KEY = process.env.MOTOR_KEY;

function headers() {
  return { "Content-Type": "application/json", Authorization: `Bearer ${MOTOR_KEY}` };
}

/** Análisis completo de un partido (PRO = profundo). forzar=true regenera. */
export async function analizarPartido(local, visitante, liga = "ligamx", profundo = false, forzar = false) {
  const r = await fetch(`${MOTOR_URL}/pickazo/analizar`, {
    method: "POST",
    headers: headers(),
    body: JSON.stringify({ local, visitante, liga, profundo, forzar }),
  });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Picks sugeridos del día (gratis). */
export async function picksDelDia(liga = "ligamx", limite = 8) {
  const r = await fetch(`${MOTOR_URL}/pickazo/picks_dia?liga=${encodeURIComponent(liga)}&limite=${limite}`,
    { headers: headers() });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Lista de partidos de una liga (para Jornadas). */
export async function partidosLiga(liga = "ligamx", limite = 60) {
  const r = await fetch(`${MOTOR_URL}/pickazo/partidos?liga=${encodeURIComponent(liga)}&limite=${limite}`,
    { headers: headers() });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Tabla de posiciones de una liga. */
export async function tablaLiga(liga = "ligamx") {
  const r = await fetch(`${MOTOR_URL}/pickazo/tabla?liga=${encodeURIComponent(liga)}`,
    { headers: headers() });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Jugadores de una liga. */
export async function jugadoresLiga(liga = "ligamx", top = 50) {
  const r = await fetch(`${MOTOR_URL}/pickazo/jugadores?liga=${encodeURIComponent(liga)}&top=${top}`,
    { headers: headers() });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Health check. */
export async function motorVivo() {
  try { const r = await fetch(`${MOTOR_URL}/pickazo/estado`); return r.ok; }
  catch { return false; }
}

/** Top jugadores de un equipo (para la pantalla del partido). */
export async function jugadoresEquipo(liga, equipo, top = 5) {
  const r = await fetch(`${MOTOR_URL}/pickazo/jugadores_equipo?liga=${encodeURIComponent(liga)}&equipo=${encodeURIComponent(equipo)}&top=${top}`,
    { headers: headers() });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Alineaciones de un partido (probables/confirmadas). */
export async function alineacionesPartido(liga, local, visitante) {
  const r = await fetch(`${MOTOR_URL}/pickazo/alineaciones?liga=${encodeURIComponent(liga)}&local=${encodeURIComponent(local)}&visitante=${encodeURIComponent(visitante)}`,
    { headers: headers() });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Lista de jornadas de una liga (para el selector). */
export async function jornadasLiga(liga = "ligamx") {
  const r = await fetch(`${MOTOR_URL}/pickazo/jornadas?liga=${encodeURIComponent(liga)}`,
    { headers: headers() });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Partidos de TODAS las ligas juntos (pestaña "Todas"). */
export async function todasLasLigas(limite = 300) {
  const r = await fetch(`${MOTOR_URL}/pickazo/todas_ligas?limite=${limite}`, { headers: headers() });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Trae la alineación de UN partido (botón por juego). */
export async function traerAlineacion(liga, local, visitante) {
  const r = await fetch(`${MOTOR_URL}/pickazo/traer_alineacion`, {
    method: "POST", headers: headers(),
    body: JSON.stringify({ liga, local, visitante }),
  });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}

/** Parlays del día armados por el motor (Camino B: fuente única).
 *  forzar=true regenera ignorando la cache del motor. */
export async function parlaysDelDia(liga = "ligamx", forzar = false) {
  const url = `${MOTOR_URL}/pickazo/parlays?liga=${encodeURIComponent(liga)}` +
    (forzar ? "&forzar=true" : "");
  const r = await fetch(url, { headers: headers() });
  if (!r.ok) throw new Error(`Motor respondió ${r.status}`);
  return await r.json();
}
